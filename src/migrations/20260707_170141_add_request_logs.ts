...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`request_logs\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`request_logs_source_idx\` ON \`request_logs\` (\`source_id\`);`)
  await db.run(sql`CREATE INDEX \`request_logs_status_idx\` ON \`request_logs\` (\`status\`);`)
  await db.run(sql`CREATE INDEX \`request_logs_updated_at_idx\` ON \`request_logs\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`request_logs_created_at_idx\` ON \`request_logs\` (\`created_at\`);`)
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`request_logs_id\` integer REFERENCES request_logs(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`request_logs\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "subscriptions_id", "sources_id", "feed_items_id", "media_id", "users_id") SELECT "id", "order", "parent_id", "path", "subscriptions_id", "sources_id", "feed_items_id", "media_id", "users_id" FROM \`payload_locked_documents_rels\`;`)
